...
- 搜索歌曲结果和网络歌单支持长按多选、批量下载
- 优化批量添加歌曲到歌单的去重和数量上限处理
- 歌单导入改为单次快速请求；返回数量与歌单实际数量不一致时，可在导入后长按歌单选择“同步更新”
- 收藏的网络歌单支持离线冷启动加载，已下载歌曲可直接从本地播放
- 优化无网络启动流程，保留本地歌曲、本地歌单和已缓存歌曲的离线使用能力
- 修复已下载文件被删除后点击歌曲无响应的问题，联网时自动回退到在线播放
