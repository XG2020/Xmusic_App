/**
 * 纯 JS MD5（RFC 1321 标准实现，无原生/第三方依赖）。
 * 用于开发者密钥校验：代码中只保留密钥的 MD5 值，不出现明文。
 */
...
function toHex(n: number): string {
  ...
  for (let i = 0; i < 4; i++) {
    s +=
      HEX_CHARS.charAt((n >> (i * 8 + 4)) & 0x0f) +
      HEX_CHARS.charAt((n >> (i * 8)) & 0x0f);
  }
  return s;
}
